...
namespace PokemonPocket
{
    public class Client
    {
        ...
        public Client(Player player, PokemonContext pokemonContext)
        {
            Player = player;
        ...
        public void Start()
        {
            ...
            {
                // Setup connection
                string addr = Insero.PromptString("Input Host address");
                int port = 8000;
                ...
                // Fight loop
                while (Pokemon.Health > 0 && EnemyPokemon.Health > 0)
                {
                    // Show scene
                    ShowScene();
